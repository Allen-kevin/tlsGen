...
//	conf->burst = (conf->connections * 1.0) / (BASE_CONNS * 1.0) * conf->burst;
	...
	config_lookup_int(&cfg, "max_prop", &(conf->max_prop));
	config_lookup_int(&cfg, "prio_prop", &(conf->prio_prop));
	config_destroy(&cfg);
	return (EXIT_SUCCESS);
}
...
{
	config_t cfg;
	unsigned int epoch, connections, prior_conns, burst, kill, max_prop, prio_prop;
	double rate;
	int flag = 0;
...
//			conf->burst = (conf->connections * 1.0) / (BASE_CONNS * 1.0) * conf->burst;
	...
	if (config_lookup_int(&cfg, "prio_prop", &prio_prop)) {
		if (prio_prop != conf->prio_prop) {
			printf("the priority proportion of unsending connections in this round updated from %d to %d\n", 
					conf->prio_prop,
					prio_prop);
			conf->prio_prop = prio_prop;
		}
	} else {
		config_destroy(&cfg);
		return (EXIT_FAILURE);
	}
	if (config_lookup_int(&cfg, "max_prop", &max_prop)) {
		if (max_prop != conf->max_prop) {
			printf("the maximum proportion of unsending connections in this round updated from %d to %d\n", 
					conf->max_prop,
					max_prop);
			conf->max_prop = max_prop;
		}
	} else {
		config_destroy(&cfg);
		return (EXIT_FAILURE);
